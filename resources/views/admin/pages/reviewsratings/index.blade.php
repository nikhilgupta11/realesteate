            ...
            <div class="row">
                <div class="page-header">
                    <h3 class="page-title">
                        ...
                        </span> Reviews and Ratings
                    </h3>
                    <nav aria-label="breadcrumb">
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item active" aria-current="page">
                                <span></span> <a href="{{ route('reviewsratings.create') }}" class="btn btn-success btn-lg float-right">Create</a>
                            </li>
                        </ul>
                    </nav>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    "render": function(data, type, full, meta) {
                        var url = "{{route('reviewsratings.update', '')}}" + "/" + full.id;
                        var show = "{{route('reviewsratings.show','')}}" + "/" + full.id;
                        var edit_url = "{{route('reviewsratings.edit', '')}}";
                        var edit_url1 = "{{ env('APP_URL') }}"+ "/admin/reviewsratings/" + full.id + "/edit";
                        return `
        ...
        $('#savebutton').on('click', function() {
            $.ajax({
                type: "POST",
                url: "{{ url('/admin/delete-reviewsrating') }}",
                data: {
                    id: nikh
                },
                dataType: 'json',
